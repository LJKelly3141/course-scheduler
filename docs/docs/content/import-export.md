...
This reference covers all import types, the preview workflow, export options, and calendar (ICS) exports. For hands-on walkthroughs, see [Importing Data from a Registrar Export](?page=tutorial-import) and [Exporting and Publishing Schedules](?page=tutorial-export).
...
Navigate to **Import / Export** in the app sidebar for imports. For exports, use the **Export** dropdown on the Schedule Grid page.
...
4. **Commit** — Select Import to save to the database
...
- **Per-instructor download** — Select the calendar icon next to any selected instructor to download their .ics file
- **Bulk download** — Select **Download All Selected Calendars** to get a ZIP file containing individual .ics files for each selected instructor
