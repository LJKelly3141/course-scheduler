...
This reference covers the end-to-end workflow for Course Scheduler. For hands-on walkthroughs, see [Setting Up Your First Term](?page=tutorial-first-term) and [Importing Data from a Registrar Export](?page=tutorial-import).
...
Go to **Terms** and select **+ Add Term**. Enter the term name (e.g., "Fall 2026"), select the type (Fall, Spring, Summer, Winter), and set start/end dates. The term starts in **Draft** mode so you can make changes freely.
...
- Select any meeting card to view details or edit
...
1. Go to **Terms** and select the **lock icon** to finalize the term
...
> **Tip:** Select the **theme toggle** in the sidebar footer to switch between light, dark, and system themes.
