...
This reference covers global application configuration including department name, academic year, export directories, database management, and GitHub Pages integration. For initial setup instructions, see [Setting Up Your First Term](?page=tutorial-first-term).
...
The folder where **Save to Local Directory** writes HTML schedule files. Select the path input to open a directory picker, or type a path directly.
...
Select **Download Backup** to save a portable copy of your entire database. This creates a standalone `.db` file you can use to restore your data or move to another machine.
...
1. Select **Set Up GitHub** (or **Reconfigure** if already connected)
