...
This reference covers creating and managing academic terms, their draft/final lifecycle, sessions, and the Copy Term feature. For hands-on walkthroughs, see [Setting Up Your First Term](?page=tutorial-first-term) and [Copying a Term for Next Year](?page=tutorial-copy-term).
...
Navigate to **Terms** in the app sidebar.
...
Select **+ Add Term** at the top. Fill in:
...
Select the **pencil icon** to edit the name, type, or dates. Editing is only available for draft terms.

### Delete a Term

Select the **trash icon** to delete a single term. Deleting a term removes all its sections and meetings. This action requires confirmation.

### Batch Delete Terms

Use the **checkboxes** on the left side of each term row to select multiple terms, then choose **Delete Selected** to remove them all at once.
...
3. Go to **Terms** and select the **lock icon** to finalize
4. The term status changes to **Final** — all editing is disabled

### Final → Draft (Unlock)

Select the **lock icon** again to unlock a finalized term and return it to draft status for further editing.

## Copy Term

Select the **Copy** button on any term to create a duplicate. Options:
...
Select **+ Add Session** to create a session manually, or use **Paste Sessions** to import from a spreadsheet.
...
Select **Paste Sessions** to import a table from your clipboard. The dialog supports:
