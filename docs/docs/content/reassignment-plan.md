...
This reference covers building and applying reassignment plans, importing from existing terms, and managing plan entries. For a hands-on walkthrough, see [Planning Reassignment Rotations](?page=tutorial-reassignment).
...
Navigate to **Reassignment Plan** under **Planning** in the app sidebar.
...
Select **+ Add** in any semester cell for an instructor already in the grid. Fill in:
...
Select **+ Add Instructor** below the grid. Choose an instructor from the dropdown, then select a semester button (Fall, Spring, Summer, or Winter) to open the add form for that cell.

## Deleting Entries

Select the **X** button on any entry to remove it from the plan.
...
Select **Apply to Term** to auto-create load adjustments in a draft term based on the plan.
...
Select **Import from Term** to extract existing load adjustments from a completed term and add them to the plan.
